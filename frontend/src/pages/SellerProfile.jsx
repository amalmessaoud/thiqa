...
export default function SellerProfile() {
  ...
  useEffect(() => {
    if (!sellerId) return;
    setLoading(true);
    thiqaApi.search(decodeURIComponent(sellerId))
      .then((result) => {
        if (result?.found) {
          setData(result);
  ...
  if (loading) return <main className="seller-page" dir="rtl"><div className="seller-loading">جاري التحميل...</div></main>;
  if (error) return <main className="seller-page" dir="rtl"><div className="seller-error">{error}</div></main>;
  if (!data) return null;

  const seller = data.seller;
  const trust = data.trust_score;
  const phone = seller?.contacts?.find((c) => c.type === "phone")?.value;
  const fbLink = seller?.contacts?.find((c) => c.type === "facebook")?.value || seller?.profile_url;
  ...
  return (
    <main className="seller-page" dir="rtl">
      ...
      <SellerInfo phone={phone} username={seller?.display_name} fbLink={fbLink} />

      <div className="seller-card">
        <h3>تحليل الحساب</h3>
        <div className="seller-fb-rows">
          <div className="seller-fb-row"><span>عمر الحساب</span><span>{formatAge(seller?.account_age_days)}</span></div>
          <div className="seller-fb-row"><span>عدد المنشورات</span><span>{seller?.post_count}</span></div>
          <div className="seller-fb-row"><span>المنصة</span><span>{seller?.platform}</span></div>
        </div>
      </div>
      ...
      <div className="seller-actions">
        ...
        <PrimaryButton fullWidth variant="green" onClick={() => navigate(`/review/${sellerId}`)}>
